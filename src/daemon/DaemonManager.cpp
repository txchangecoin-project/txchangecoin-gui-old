...
{
    // prepare command line arguments and pass to txchangecoind
    ...
    qDebug() << "starting txchangecoind " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM txchangecoind.exe");
#else
                QProcess::execute("pkill txchangecoind");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./txchangecoind status` returns BUSY when syncing.
...
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/txchangecoind.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/txchangecoind";
#endif
