...
MONERO_DIR=txchangecoin
MONEROD_EXEC=txchangecoind
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/txchangecoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=txchangecoind.exe
fi
...
$QMAKE ../txchangecoin-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy txchangecoind to bin folder
