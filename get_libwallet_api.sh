#!/bin/bash
MONERO_URL=https://github.com/txchangecoin-project/txchangecoin.git
...
MONERO_DIR=$ROOT_DIR/txchangecoin
BUILD_LIBWALLET=false

# init and update txchangecoin submodule
if [ ! -d $MONERO_DIR/src ]; then
    git submodule init txchangecoin
fi
...
get_tag
popd
# create local txchangecoin branch
git -C $MONERO_DIR checkout -B $VERSIONTAG

# Merge txchangecoin PR dependencies
...
git -C $MONERO_DIR config user.name "Txchangecoin GUI"
git -C $MONERO_DIR config user.email "gui@txchangecoin.local"
# check for PR requirements in most recent commit message (i.e requires #xxxx)
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging txchangecoin push request #$PR"
    # fetch pull request and merge
    git -C $MONERO_DIR fetch origin pull/$PR/head:PR-$PR
    git -C $MONERO_DIR merge --quiet PR-$PR  -m "Merge txchangecoin PR #$PR"
...
# Build libwallet if no previous version file exists
    ...
    echo "txchangecoin/version.h not found - Building libwallet"
    BUILD_LIBWALLET=true
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_MONERO_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_MONERO_VERSION) is already built. Remove txchangecoin/lib/libwallet_merged.a to force rebuild"
    fi
fi
...
echo "cleaning up existing txchangecoin build dir, libs and includes"
