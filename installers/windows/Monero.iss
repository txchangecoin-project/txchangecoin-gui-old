; Txchangecoin Boron Butterfly GUI Wallet Installer for Windows
; Copyright (c) 2017-2019, The Txchangecoin Project
; See LICENSE

[Setup]
AppName=Txchangecoin GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.14.0.0
DefaultDirName={pf}\Txchangecoin GUI Wallet
DefaultGroupName=Txchangecoin GUI Wallet
UninstallDisplayIcon={app}\txchangecoin-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The Txchangecoin Developer Community
AppPublisherURL=https://txchange.online
...
; Background info:
; This installer indeed asks for admin rights so the txchangecoin files can be copied to a place where they have at least
...
; As of version 0.14.0.0, the txchangecoin .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to txchangecoin.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; txchangecoin GUI wallet exe and guide
Source: "bin\txchangecoin-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\txchangecoin-GUI-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; txchangecoin GUI wallet log file
...
Source: "txchangecoin-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; txchangecoin CLI wallet
Source: "bin\txchangecoin-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\txchangecoin-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; txchangecoin wallet RPC interface implementation
Source: "bin\txchangecoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; txchangecoin daemon
Source: "bin\txchangecoind.exe"; DestDir: "{app}"; Flags: ignoreversion

; txchangecoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "txchangecoin-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; txchangecoin blockchain utilities
Source: "bin\txchangecoin-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\txchangecoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\txchangecoin-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\txchangecoin-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\txchangecoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\txchangecoin-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\txchangecoin-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0 and after
; Source: "bin\txchangecoin-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\txchangecoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\txchangecoin');
  s := 'The default folder to store the txchangecoin blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\txchangecoin-project\txchangecoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" txchangecoin default location
...
begin
  Result := BlockChainDir('') + '\txchangecoin.log';
...
var s: String;
begin
  s := ExpandConstant('{app}\txchangecoin-wallet-gui.log');
  if Pos(' ', s) > 0 then begin
    // Quotes needed for filename with blanks
    s := '"' + s + '"';
  end;
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "txchangecoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the txchangecoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\txchangecoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\txchangecoin-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "txchangecoin GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\txchangecoin-wallet-gui.exe"; Parameters: {code:WalletFlags}
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\txchangecoin-GUI-guide.pdf"; IconFilename: "{app}\txchangecoin-wallet-gui.exe"
...
Name: "{group}\Utilities\Txchangecoin Daemon"; Filename: "{app}\txchangecoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\txchangecoin-wallet-cli.exe"; WorkingDir: "{userdocs}\Txchangecoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Txchangecoin\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\txchangecoin-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\txchangecoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM txchangecoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\txchangecoin-wallet-gui.exe"; Parameters: {code:WalletFlags}; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\txchangecoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\txchangecoin-project\txchangecoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\txchangecoin-project\txchangecoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};
