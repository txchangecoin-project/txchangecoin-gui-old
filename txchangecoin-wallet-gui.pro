...
# cleaning "auto-generated" txchangecoin directory on "make distclean"
...
linux {
   ...
   #     }
    ...
    # currently txchangecoin has an issue with "static" build and linunwind-dev,
...
android{
    deploy.commands += make install INSTALL_ROOT=$$DESTDIR && androiddeployqt --input android-libtxchangecoin-wallet-gui.so-deployment-settings.json --output $$DESTDIR --deployment bundled --android-platform android-21 --jdk /usr/lib/jvm/java-8-openjdk-amd64 -qmldir=$$PWD
}
...
DISTFILES += \
    notes.txt \
    txchangecoin/src/wallet/CMakeLists.txt
