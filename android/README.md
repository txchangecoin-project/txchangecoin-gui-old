Copyright (c) 2014-2018, The Txchangecoin Project
...
# Base environnement
        ...
        cd txchangecoin/utils/build_scripts
        docker build -f android32.Dockerfile -t txchangecoin-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t txchangecoin-gui-android .
        docker create -it --name txchangecoin-gui-android txchangecoin-gui-android bash

# Get the apk

        docker cp txchangecoin-gui-android:/opt/android/txchangecoin-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/txchangecoin-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
